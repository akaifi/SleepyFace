...
(function() {
    ...
    window.addEventListener('load', function() {
        ...
        const faceEl1 = document.getElementsByClassName('sleepyFace1');
        const faceEl2 = document.getElementsByClassName('sleepyFace2');
        const faceEl3 = document.getElementsByClassName('sleepyFace3');
        const faceEl4 = document.getElementsByClassName('sleepyFace4');
        // faces.forEach(new AirHorn(face));
        const installEl = document.getElementById('installer');
        // const ShowStart = document.getElementById("myForm");
        const airhorn = new AirHorn(airhornEl);
        const face1 = new AirHorn(faceEl1);
        const face2 = new AirHorn(faceEl2);
        const face3 = new AirHorn(faceEl3);
        const face4 = new AirHorn(faceEl4);
        ...
        if (Comlink && window.opener) {
            ...
            Comlink.expose(face1, port1);
            Comlink.expose(face2, port1);
            Comlink.expose(face3, port1);
            Comlink.expose(face4, port1);
        ...
        if (navigator.presentation && navigator.presentation.receiver) {
            navigator.presentation.receiver.connectionList.then(list => {
                list.connections.forEach(connection => {
                    ...
                    Comlink.expose(face1, MessageChannelAdapter.wrap(connection));
                    Comlink.expose(face2, MessageChannelAdapter.wrap(connection));
                    Comlink.expose(face3, MessageChannelAdapter.wrap(connection));
                    Comlink.expose(face4, MessageChannelAdapter.wrap(connection));
                });
                list.onconnectionavailable = event => {
                    Comlink.expose(airhorn, MessageChannelAdapter.wrap(event.connection));
                    Comlink.expose(face1, MessageChannelAdapter.wrap(event.connection));
                    Comlink.expose(face2, MessageChannelAdapter.wrap(event.connection));
                    Comlink.expose(face3, MessageChannelAdapter.wrap(event.connection));
                    Comlink.expose(face4, MessageChannelAdapter.wrap(event.connection));
                };
            });
        }
        ...
        if (location.hash == '#instant') {
            ...
            face1.start({ loop: false });
            face2.start({ loop: false });
            face3.start({ loop: false });
            face4.start({ loop: false });
        }
